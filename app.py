...
def init_gemini(api_key=None):
    ...
    if key:
        ...
        try:
            ...
            # Prefer Flash for OCR, then Pro
            target_order = ["flash", "pro"]
...
@app.route("/api/customer/block_credit_card", methods=["POST"])
def block_credit_card():
    ...
    conn = get_db()
    conn.execute(
        "INSERT INTO card_blocks (customer_uuid, card_type, action, reason, status, created_at) VALUES (?, ?, ?, ?, ?, ?)",
        (cust_id, "CREDIT", "BLOCKED", reason, "ACTIVE", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    )
    conn.commit()
    conn.close()
    ...
    return jsonify({
        "success": True,
        "message": f"Credit card blocked for customer {cust_id}",
        "card_type": "CREDIT",
        "status": "BLOCKED"
    })

@app.route("/api/customer/block_debit_card", methods=["POST"])
def block_debit_card():
    ...
    conn.execute(
        "INSERT INTO card_blocks (customer_uuid, card_type, action, reason, status, created_at) VALUES (?, ?, ?, ?, ?, ?)",
        (cust_id, "DEBIT", "BLOCKED", reason, "ACTIVE", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    )
    conn.commit()
    conn.close()
    ...
    return jsonify({
        "success": True,
        "message": f"Debit card blocked for customer {cust_id}",
        "card_type": "DEBIT",
        "status": "BLOCKED"
    })

@app.route("/api/customer/freeze_account", methods=["POST"])
def freeze_account():
    data = request.json
    cust_id = data.get("customer_uuid")
    reason = data.get("reason", "Account security measure")
    
    conn = get_db()
    conn.execute(
        "INSERT INTO account_freezes (customer_uuid, freeze_type, reason, status, created_at) VALUES (?, ?, ?, ?, ?)",
        (cust_id, "FULL", reason, "ACTIVE", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    )
    conn.commit()
    conn.close()
    ...
    return jsonify({
        "success": True,
        "message": f"Account frozen for customer {cust_id}",
        "freeze_type": "FULL",
        "status": "FROZEN"
    })

@app.route("/api/customer/card_status/<cust_id>", methods=["GET"])
def get_card_status(cust_id):
    conn = get_db()
    
    credit_block = conn.execute(
        "SELECT status FROM card_blocks WHERE customer_uuid = ? AND card_type = 'CREDIT' ORDER BY created_at DESC LIMIT 1",
        (cust_id,)
    ).fetchone()
    
    debit_block = conn.execute(
        "SELECT status FROM card_blocks WHERE customer_uuid = ? AND card_type = 'DEBIT' ORDER BY created_at DESC LIMIT 1",
        (cust_id,)
    ).fetchone()
    
    account_freeze = conn.execute(
        "SELECT status FROM account_freezes WHERE customer_uuid = ? ORDER BY created_at DESC LIMIT 1",
        (cust_id,)
    ).fetchone()
    ...
    return jsonify({
        "credit_card_status": credit_block[0] if credit_block else "ACTIVE",
        "debit_card_status": debit_block[0] if debit_block else "ACTIVE",
        "account_status": account_freeze[0] if account_freeze else "ACTIVE"
    })
...
def api_chatbot_query():
    ...
    if not match_cust.empty:
        reply_parts.append("## Customer Investigation Results\n")
        for _, row in match_cust.head(3).iterrows():
            ...
            reply_parts.append(f"### **{row['full_name']}**")
            reply_parts.append(f"- **UPI ID:** {row['upi_id']}")
            reply_parts.append(f"- **Email:** {row['email']}")
            reply_parts.append(f"- **Account Balance:** ₹{float(row.get('account_balance', 0)):,.2f}")
            reply_parts.append(f"- **Risk Level:** **{risk_level}**")
            reply_parts.append(f"- **Anomalies Found:** {len(anomalies)}")
            
            if anomalies:
                reply_parts.append(f"\n#### Flagged Transactions:")
                for anom in anomalies[:5]:  # Show top 5 anomalies
                    txn = anom.get("transaction", {})
                    merchant = anom.get("merchant_name", "Unknown")
                    reply_parts.append(f"\n**Transaction:** {txn.get('transaction_uuid', 'N/A')}")
                    reply_parts.append(f"- **Amount:** ₹{float(txn.get('transaction_amount', 0)):,.2f}")
                    reply_parts.append(f"- **Merchant:** {merchant}")
                    reply_parts.append(f"- **Status:** {txn.get('transaction_status', 'N/A')}")
                    reply_parts.append(f"- **Risk Score:** {anom.get('risk_score', 0)}")
                    
                    for issue in anom.get("anomalies", []):
                        severity_emoji = {"CRITICAL": "🔴", "HIGH": "🟠", "MEDIUM": "🟡", "LOW": "🟢"}.get(issue.get("severity"), "")
                        reply_parts.append(f"  {severity_emoji} **{issue.get('type', 'UNKNOWN')}:** {issue.get('detail', '')}")
            else:
                reply_parts.append(f"✓ No anomalies detected for this customer.")
    ...
    if txn_ids:
        reply_parts.append("## Transaction Details\n")
        for tid in txn_ids:
            ...
            if not t_row.empty:
                ...
                reply_parts.append(f"### **{tid}**")
                reply_parts.append(f"- **Amount:** ₹{float(t_data.get('transaction_amount', 0)):,.2f}")
                reply_parts.append(f"- **Customer UUID:** {t_data.get('customer_uuid', 'N/A')}")
                reply_parts.append(f"- **Status:** {t_data.get('transaction_status', 'N/A')}")
                reply_parts.append(f"- **Device ID:** {t_data.get('customer_device_id', 'N/A')}")
                reply_parts.append(f"- **IP Address:** {t_data.get('customer_ip_address', 'N/A')}")
                reply_parts.append(f"- **Timestamp:** {t_data.get('transaction_timestamp', 'N/A')}")
                reply_parts.append("")

    # 3. Search for specific Cheque ID (CHQxxxxxx)
    chq_ids = re.findall(r'CHQ\d+', message.upper())
    if chq_ids and cheques_df is not None:
        reply_parts.append("## Cheque Analysis\n")
        for cid in chq_ids:
            ...
            if not c_row.empty:
                ...
                reply_parts.append(f"### **{cid}**")
                reply_parts.append(f"- **Issuer:** {c_data.get('issuer_name', 'N/A')}")
                reply_parts.append(f"- **Beneficiary:** {c_data.get('beneficiary_name', 'N/A')}")
                reply_parts.append(f"- **Amount:** ₹{float(c_data.get('cheque_amount', 0)):,.2f}")
                reply_parts.append(f"- **Status:** {c_data.get('cheque_status', 'N/A')}")
                
                if c_data.get('is_tampered'):
                    reply_parts.append(f"- **🔴 TAMPERED:** YES")
                    reply_parts.append(f"- **Tamper Type:** {c_data.get('tamper_type', 'N/A')}")
                    reply_parts.append(f"- **Confidence:** {c_data.get('tamper_confidence', 0)}%")
                else:
                    reply_parts.append(f"- **✓ Authentic**")
                reply_parts.append("")

    # 4. Search for cheques by Issuer/Beneficiary name
    if cheques_df is not None and not chq_ids:
        ...
        if not match_chq.empty:
            reply_parts.append("## Cheque Records Found\n")
            for _, row in match_chq.head(3).iterrows():
                found_entities.append(f"Cheque record for {row['issuer_name']} to {row['beneficiary_name']}")
                reply_parts.append(f"### **{row['cheque_number']}**")
                reply_parts.append(f"- **From:** {row['issuer_name']}")
                reply_parts.append(f"- **To:** {row['beneficiary_name']}")
                reply_parts.append(f"- **Amount:** ₹{float(row.get('cheque_amount', 0)):,.2f}")
                reply_parts.append(f"- **Status:** {row.get('cheque_status', 'N/A')}")
                if row.get('is_tampered'):
                    reply_parts.append(f"- **🔴 ALERT - TAMPERED:** {row.get('tamper_type', 'Unknown')}")
                reply_parts.append("")

    # 5. Generic fraud analysis if no specific entities found
    if not found_entities:
        # Check if user is asking for general statistics or overview
        if any(word in message.lower() for word in ["flagged", "risky", "suspicious", "analyze", "overview", "summary"]):
            reply_parts.append("## System Overview\n")
            reply_parts.append(f"- **Total Customers:** {len(customers_df)}")
            reply_parts.append(f"- **Total Transactions:** {len(transactions_df)}")
            if cheques_df is not None:
                reply_parts.append(f"- **Total Cheques:** {len(cheques_df)}")
                tampered = cheques_df[cheques_df.get('is_tampered') == True]
                if not tampered.empty:
                    reply_parts.append(f"- **🔴 Tampered Cheques:** {len(tampered)}")
            
            # Find high-risk customers
            all_risks = []
            for _, cust in customers_df.iterrows():
                _, _, anomalies = detect_anomalies(customer_uuid=cust["customer_uuid"])
                if anomalies:
                    all_risks.append((cust["full_name"], cust["upi_id"], len(anomalies), get_risk_level(anomalies)))
            
            if all_risks:
                all_risks.sort(key=lambda x: x[2], reverse=True)
                reply_parts.append(f"\n#### Top Flagged Customers:")
                for name, upi, count, risk in all_risks[:5]:
                    reply_parts.append(f"- {name} ({upi}) - {count} anomalies - Risk: {risk}")
        else:
            reply_parts.append("No matching customer, transaction, or cheque records found. Try searching by:")
            reply_parts.append("- Customer name, UPI ID, or email")
            reply_parts.append("- Transaction ID (e.g., TXN12345)")
            reply_parts.append("- Cheque number (e.g., CHQ12345)")
            reply_parts.append("- Or ask for 'Analyze Flagged' or 'Overview'")

    reply_text = "\n".join(reply_parts) if reply_parts else "No results found."
